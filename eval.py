    ...
    with tqdm(total=len(gallery_loader)+len(query_loader)+4+len(gallery_loader)*gallery_loader.batch_size, desc="Running Eval") as pbar:
        ...
        with torch.inference_mode():
            ...
            for (image,label) in query_loader:
                ...
                pbar.update(1)
            gallery_embs=F.normalize(torch.cat(gallery_embs),-1).to(device)
            gallery_labels=torch.cat(gallery_labels).flatten().to(device)
            query_embs=F.normalize(torch.cat(query_embs),-1).to(device)
            ...
            for k in [1,5,10,20]:
                ...
                eval_board[f'Top-{k}-Precision']=(((gallery_labels[index]==query_labels.unsqueeze(-1)).sum())/(k*cosine_sim.size(0))).item()
                torch.cuda.empty_cache()
                pbar.update(1)
            mAP=0
            for k in range(1,cosine_sim.size(-1)+1):
                _,index=cosine_sim.topk(k)
                mAP+=(((gallery_labels[index]==query_labels.unsqueeze(-1)).sum())/(k*cosine_sim.size(0))).item()
                torch.cuda.empty_cache()
                pbar.update(1)
